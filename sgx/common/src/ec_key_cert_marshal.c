                           ...
                           int *ec_der_bytes_out_len)
{
  ...
  {
    // PKEY to be marshalled is not of EC type
    return EXIT_FAILURE;
  }
  ...
  {
    // initially non-NULL pointer to DER result buffer
    return EXIT_FAILURE;
  }
  ...
  if (out_len <= 0)
  {
    // PKEY to DER format conversion error
    return EXIT_FAILURE;
  }
                           ...
                           int *ec_der_bytes_out_len)
{
  ...
  if (out_len <= 0)
  {
    // X509 to DER format conversion error
    return EXIT_FAILURE;
  }
